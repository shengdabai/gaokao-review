...
async function handleTutor(req: VercelRequest, res: VercelResponse, userId: string) {
    ...
    if (sessionId) {
        const existing = db.prepare('SELECT id FROM tutor_sessions WHERE id = ? AND user_id = ?').get(sessionId, userId);
    ...
    if (session.isNew) {
        db.prepare('INSERT INTO tutor_sessions (id, user_id, subject, topic, session_type, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
            .run(session.id, userId, subject, topic || null, mode, 'active', new Date().toISOString(), new Date().toISOString());
    }

    db.prepare('INSERT INTO tutor_messages (id, session_id, role, content, message_type, created_at) VALUES (?, ?, ?, ?, ?, ?)')
        .run(generateId(), session.id, 'user', message.trim(), 'chat', new Date().toISOString());

    const history = db.prepare('SELECT role, content FROM tutor_messages WHERE session_id = ? ORDER BY created_at DESC LIMIT 10').all(session.id) as any[];
...
请用中文回复（约200字），结尾提出引导性问题。`;
    ...
    db.prepare('INSERT INTO tutor_messages (id, session_id, role, content, message_type, created_at) VALUES (?, ?, ?, ?, ?, ?)')
        .run(generateId(), session.id, 'assistant', responseText, 'chat', new Date().toISOString());
...
async function handlePredict(req: VercelRequest, res: VercelResponse, userId: string) {
    ...
    try {
        const weak = db.prepare('SELECT topic FROM study_progress WHERE user_id = ? AND subject = ? AND mastery_level < 60 LIMIT 10').all(userId, subject) as any[];
        weakPoints = weak.map(t => t.topic);
    } catch (e) { }
