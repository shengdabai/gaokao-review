 ...
 * 
 * 调用 Get笔记 API，失败时降级到 Gemini AI 生成
 */
 ...
 * 保存搜索历史
 */
function saveSearchHistory(userId: string, query: string, subject?: string): void {
  try {
    const db = getDatabase();
    db.prepare(`
      INSERT INTO search_history (id, user_id, query, subject, created_at)
      VALUES (?, ?, ?, ?, ?)
    `).run(generateId(), userId, query, subject || null, new Date().toISOString());
  } catch (error) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 保存搜索历史
    saveSearchHistory(auth.userId, query.trim(), subject);
